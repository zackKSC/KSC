...
def login():
    ...
    def quiz_easy():
        ...
        def next_question():
            ...
            def end_page():
                ...
                global result_image
                result_image=tk.PhotoImage(file="result.png")
                label=tk.Label(window,image=result_image)
                label.place(x=130, y=1)
                
                correct_q_num = Label(window, text = "점수 : " + str(correct_q * 10), font=("나눔바른펜", 16, "bold"))
                correct_q_num.place(x=50, y=100)
                ave_score = Label(window, text = "평균 점수 : " + str(new_ave_score), font=("나눔바른펜", 16, "bold"))
                ave_score.place(x=50, y=150)
                play_num_l = Label(window, text = "도전 횟수 : " + str(play_num+1), font=("나눔바른펜", 16, "bold"))
                play_num_l.place(x=50, y=200)
                your_mileage = Label(window, text = "마일리지 : " + str(mileage + plus_mileage), font=("나눔바른펜", 16, "bold"))
            ...
            def countstart():
                ...
                count += 1
                time += 1
                label["text"] = str(count+1) + "sec"
                if time > 4:
                    next_question()
                    return
                window.after(1000, countstart)
                
            label=Label(window, text="0sec", font=("나눔바른펜", 17, "bold"), fg=HARD_COL, bg=BGCOLOR)
            label.place(x=0, y=6)
            window.after(1000, countstart)
        ...
        buttons = []
        for i in range(4):
            btn = Button(window, text=f"{i}번", width=35, height=2, font=("나눔바른펜", 15, "bold"), bg=BTN_COLOR)
            btn.pack()
            buttons.append(btn)

        next_btn = Button(window, text="선택하기", width=15, height=2, command=select_question, font=("나눔바른펜", 15, "bold"), bg=CORRECT_COLOR)
    ...
    def quiz_normal():
        ...
        conn = pymysql.connect(host="193.123.231.213", user="st02",password="djm06178RE!",database="db_st02")
        cur = conn.cursor()
        cur.execute("select * from normal_quiz")
        rows = cur.fetchall()
    ...
    def quiz_hard():
        ...
        conn = pymysql.connect(host="193.123.231.213", user="st02",password="djm06178RE!",database="db_st02")
        cur = conn.cursor()
        cur.execute("select * from hard_quiz")
        rows = cur.fetchall()
    ...
    quiz_btn = Button(window, text="어려움", width=5, height=1, font=("나눔바른펜", 11, "bold"),fg="white", bg=HARD_COL, command = lambda: [quiz_hard(), window.destroy()])
    quiz_btn.place(x=120, y=48)
    quiz_btn = Button(window, text="보 통", width=5, height=1, font=("나눔바른펜", 11, "bold"),fg="white", bg=login_col, command = lambda: [quiz_normal(), window.destroy()])
